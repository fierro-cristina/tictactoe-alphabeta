/*Based on the pseudocode algorithm presented in Wikipedia
 *author Jazmin Cristina © 2019*/
...
// Check if a position is occupied or invalid
...
int main(){
	...
	while (!game_is_done(board)){
		...
		if (position_occupied(board, std::make_pair(row, col))){
			cout << "The position (" << row << ", " << col << ") is occupied or invalid. Please try another one." << endl;
			continue;
		}
		else{
